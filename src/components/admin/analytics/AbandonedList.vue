...
import type { AbandonedItem } from "../../../services/AnalyticsService";

defineProps<{ items: AbandonedItem[]; total: number }>();
...
function fmtMoney(v: number | null, currency: string | null): string {
  if (v == null) return "—";
  return "$" + Math.round(v).toLocaleString("es-CO") + (currency ? " " + currency : "");
}

/** Enlace al WhatsApp existente con un mensaje de recuperación de carrito prellenado. */
function whatsappLink(it: AbandonedItem): string | null {
  const raw = it.contact?.whatsapp;
  if (!raw) return null;
  const digits = raw.replace(/\D/g, "");
  if (!digits) return null;
  const msg = `¡Hola${it.contact?.name ? " " + it.contact.name : ""}! Vimos que dejaste "${it.cart}" en tu carrito. ¿Te ayudamos a completar la compra?`;
...
<template>
  <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
      <h3 class="text-sm font-bold text-gray-700">Carritos abandonados</h3>
    ...
    <div v-if="items.length === 0" class="p-6 text-center text-sm text-gray-400">
      No hay checkouts abandonados en este rango (InitiateCheckout sin Purchase). Lo alimenta F3.
    </div>
      ...
      <table class="w-full text-sm min-w-[640px]">
        <thead>
          <tr class="text-[11px] uppercase text-gray-400 tracking-wide border-b border-gray-100">
            ...
            <th class="text-left font-semibold px-2 py-2">Carrito</th>
            <th class="text-right font-semibold px-2 py-2">Valor</th>
            <th class="text-left font-semibold px-2 py-2">Canal</th>
            <th class="text-left font-semibold px-2 py-2">Contacto</th>
        ...
        <tbody>
          ...
          >
            <td class="px-4 py-2.5 text-gray-500 whitespace-nowrap">{{ fmtDate(it.last_activity) }}</td>
            <td class="px-2 py-2.5 text-gray-700 font-medium truncate max-w-[200px]">{{ it.cart }}</td>
            <td class="px-2 py-2.5 text-right font-semibold text-gray-800 tabular-nums whitespace-nowrap">
              {{ fmtMoney(it.value, it.currency) }}
            </td>
            ...
            <td class="px-2 py-2.5 text-gray-600">
              <template v-if="it.contact">
                <div class="truncate max-w-[160px] font-medium text-gray-700">{{ it.contact.name || "—" }}</div>
                <div class="text-[11px] text-gray-400 truncate max-w-[160px]">{{ it.contact.email }}</div>
              </template>
              <span v-else class="text-gray-300 italic">anónimo</span>
            </td>
