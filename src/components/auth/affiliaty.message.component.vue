...
const borderClasses = props.variant === 'card'
    ? 'border-t border-gray-200 text-gray-600 hover:border-blue-400 hover:text-blue-600'
    : 'border-b border-[#163757] hover:border-[#ff7987]';
</script>



<template>
    <div v-if="storeAuth.getProfile()?.user" @click="validateLinkAfiliaty(id_category)" class="cursor-pointer transition-colors" :class="borderClasses">
        <div class="flex gap-3 justify-center items-center py-3">
            <div>
                <p class="font-light">Compartir link de afiliado</p>
            </div>
            <div class="shrink-0">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-current">
                    <path
