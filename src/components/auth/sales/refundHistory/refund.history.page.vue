...
const selectRefund = ref<IRefund | null>(null)
...
const fetchRefunds = async () => {
    ...
    try {
        const res = await RefundService.getRefundsByUserAndDate(dateInicio, dateFin);
        refunds.value = res;
    } catch (error) {
...
const onSelectRefund = (refund: IRefund) => {
...
<template>
    <div class="relative">
        <div class="grid gap-1 mt-3">
            <div>
                <h2 class="text-center text-xl font-bold">Historial de reembolsos</h2>
                <br>
                <h4 class="px-3 text-center">Selecciona el periodo de reembolso</h4>
        ...
        <div v-if="selectRefund !== null" class="absolute h-screen w-screen top-0 flex justify-center bg-[#0000007a]">
            <div class="bg-white rounded-sm px-4 pb-4 w-[350px] h-fit mt-10">
                ...
                <RefundComponentEmergent :item="selectRefund" />
            </div>
        </div>
    </div>
</template>