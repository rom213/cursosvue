...
<template>
    <div class="relative">
        <div class="border border-black flex p-1 px-2 rounded-sm">
            ...
            <input v-model="dataInput" class="w-40 outline-none" placeholder="Que quieres aprender" type="text">
        </div>
        <div v-if="dataReseived.length" class="bg-white z-20  max-h-28 w-[70vw] absolute px-4 pt-2 rounded-b-xs shadow pb-2 grid gap-2">
            <div class="flex cursor cursor-pointer justify-between" v-for="(cat, index) in dataReseived" :key="index" @click="handleClickItem(cat.id)">
                {{ cat.titulo }}
