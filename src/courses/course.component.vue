...
const buyCategory = (item: ICategory) => {
    if (!userAuth.getProfile()) {
        router.push("/login");
        return;
    }

    generatePayUForm(item.precio_desc, item.titulo, userAuth.getProfile()?.user?.email, item.signature, item.reference_code);
};
                ...
                <div class="px-1 grid">
                    ...
                    <div class="grid gap-2 pt-1">
                        <button @click="addCarCategory(item)" class="bg-[#2BA0FF] rounded-sm p-1">Agregar
                            carrito</button>
                        <button @click="buyCategory(item)" class="bg-[#CDFF00] rounded-sm p-1">
                            <p >
